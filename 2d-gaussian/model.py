...
class GSI:
	...
	def inspect_regressor(self):
		...
		for i in range(num_images):
			...
			plt.imshow(M_regression_val[i], cmap='jet')
			if i < num_cols:
				plt.title('Reference')
			plt.subplot(num_rows, 2*num_cols, 2*i+2)
			plt.grid(False), plt.xticks([]), plt.yticks([])
			plt.imshow(M_regression_val_hat[i], cmap='jet')
			if i < num_cols:
				plt.title('Predicted.')
		plt.tight_layout()
		plt.show()
	...
	def get_inversion(self, d_obs, m_ref):
		...
		ax.imshow(np.squeeze(m_ref), cmap='jet', vmin=np.min(m_ref), vmax=np.max(m_ref))
		plt.grid(False), plt.xticks([]), plt.yticks([])
		plt.title('Reference')
		
		ax = fig.add_subplot(gs[0, 1])
		ax.imshow(np.squeeze(m_ref_hat), cmap='jet', vmin=np.min(m_ref), vmax=np.max(m_ref))
		plt.grid(False), plt.xticks([]), plt.yticks([])
		plt.title('Inversion')
